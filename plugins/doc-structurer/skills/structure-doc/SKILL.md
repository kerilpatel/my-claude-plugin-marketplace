...
allowed-tools: Read, Write, Glob, Bash(mkdir *), Bash(tectonic *), Bash(ls *), Bash(git *)
...
Present: the JD summary, the categorized keyword list, which keywords were woven in vs. not reflected, the new folder path, the compile result (success + PDF path, or the raw tectonic error), the row added to `APPLICATIONS.md`, and — if Step 10 pushed — the GitHub blob link to the PDF (or a note that the push was skipped/failed and why).
